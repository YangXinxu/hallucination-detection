...
def main(cfg: DictConfig) -> None:
    ...
    # Check if dataset already has splits configured
    splits_configured = dataset_dict.get('splits') is not None and len(dataset_dict. get('splits', [])) > 0
    if splits_configured: 
        logger.info(f"Dataset has splits configured: {dataset_dict. get('splits')}")
        logger.info("Skipping split_dataset - dataset already has split information.")
        return
